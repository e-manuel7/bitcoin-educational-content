---
name: Programu ya Blockstream - Onchain
description: Sanidi Blockstream App kwenye simu ya mkononi na udhibiti miamala ya onchain
---
![cover](assets/cover.webp)


## 1. Utangulizi


### 1.1 Lengo la mafunzo





- Mafunzo haya yanafafanua jinsi ya kutumia programu ya simu ya **Blockstream** ili kudhibiti Bitcoin **onchain** Wallet, yaani, miamala iliyorekodiwa moja kwa moja kwenye Blockchain Bitcoin kuu.
- Inashughulikia usakinishaji, usanidi wa awali, uundaji wa Software Wallet, na shughuli za kupokea na kutuma bitcoins.
- Kumbuka: Mafunzo mengine katika Viambatisho yanashughulikia Liquid, Tazama Pekee na toleo la eneo-kazi.



![image](assets/fr/01.webp)



### 1.2 Watazamaji walengwa





- **Wanaoanza**: Watumiaji wanaotaka kudhibiti bitcoins zao kwa kutumia programu angavu ya simu.
- **Watumiaji wa kati**: Watu wanaotafuta kuelewa utendakazi wa onchain na chaguo za faragha kama vile Tor au SPV.



### 1.3. Vikumbusho kuhusu Hot wallet





- **Hot Wallet**, **Software Wallet**, **Wallet simu**, **Software Wallet**: majina yote ya programu iliyosakinishwa kwenye simu mahiri, kompyuta au kifaa chochote kilichounganishwa na Mtandao, kuwezesha funguo za kibinafsi kutoka kwa Bitcoin Wallet kudhibitiwa na kulindwa.
- Tofauti na **wallet za maunzi**, pia hujulikana kama **Cold wallet**, ambazo hutenga funguo nje ya mtandao, wallet za programu hufanya kazi katika mazingira yaliyounganishwa, na kuzifanya ziwe hatarini zaidi kwa mashambulizi ya mtandaoni.





- Matumizi yanayopendekezwa** :
    - Inafaa kwa ajili ya kudhibiti kiasi cha wastani cha Bitcoin, hasa kwa shughuli za kila siku.
    - Inafaa kwa wanaoanza au watumiaji walio na vipengee vichache, ambao Hardware Wallet inaweza kuonekana kuwa ya kupita kiasi.





- **Blocks**: Usalama mdogo kwa kuhifadhi pesa nyingi au akiba ya muda mrefu. Katika kesi hii, chagua Hardware Wallet.




## 2. Kuanzisha Programu ya Blockstream





- **Blockstream App** ni programu ya simu (iOS, Android) na eneo-kazi kwa ajili ya kudhibiti jalada na mali za Bitcoin kwenye Liquid Network. Ilinunuliwa na [Blockstream](https://blockstream.com/) mwaka wa 2016, awali iliitwa *Green Address* na kisha *Blockstream Green*.
- **Vipengele muhimu ** :
    - Shughuli za **Onchain** kwenye Blockchain Bitcoin.
    - Shughuli za mtandao **Liquid** (Sidechain kwa ubadilishanaji wa haraka na wa siri).
    - **Kuangalia pekee** kwingineko za ufuatiliaji wa fedha bila ufikiaji wa funguo.
    - Chaguzi za faragha: muunganisho kupitia **Tor**, unganisho kwa **node ya kibinafsi** kupitia Electrum, au uthibitishaji wa **SPV** ili kupunguza utegemezi kwa node za watu wengine.
    - Kazi **Replace-by-fee (RBF)** ili kuharakisha miamala ambayo haijathibitishwa.
- **Utangamano**: Huunganisha wallet za maunzi kama vile **Blockstream Jade**.
- **Interface**: Intuitive kwa wanaoanza, na chaguo za kina kwa wataalam.
- **Kumbuka**: Mwongozo huu unazingatia matumizi ya onchain. Mafunzo mengine katika Viambatisho yanashughulikia Liquid, watch-only na toleo la eneo-kazi.



## 3. Kusakinisha na kusanidi Programu ya Blockstream



### 3.1. pakua





- **Kwa Android** :
    - Pakua [Programu ya Blockstream](https://play.google.com/store/apps/details?id=com.greenaddress.greenbits_android_wallet) kutoka kwenye Duka la Google Play.
    - Mbadala: Sakinisha kupitia faili ya APK inayopatikana kwenye [GitHub rasmi ya Blockstream](https://github.com/Blockstream/green_android).
- **Kwa iOS** :
    - Pakua [Programu ya Blockstream](https://apps.apple.com/us/app/Green-Bitcoin-Wallet/id1402243590) kutoka kwa App Store.
- **Kumbuka**: Hakikisha umepakua kutoka kwa vyanzo rasmi ili kuepuka programu za ulaghai.



### 3.2. usanidi wa awali





- **Skrini ya nyumbani**: Inapofunguliwa mara ya kwanza, programu inaonyesha skrini bila Wallet iliyosanidiwa. Kwingineko zilizoundwa au zilizoletwa zitaonekana hapa baadaye.



![image](assets/fr/02.webp)





- **Badilisha mipangilio kukufaa**: Bofya "Mipangilio ya programu", rekebisha chaguo zilizo hapa chini, bofya "Hifadhi", anzisha upya programu na uunde kwingineko yako.



![image](assets/fr/03.webp)



#### 3.2.1. Faragha iliyoimarishwa (Android pekee)





- **Kazi**: Huzima picha za skrini, huficha onyesho la kukagua programu kwenye kidhibiti cha kazi, na hufunga ufikiaji simu ikiwa imefungwa.
- **Kwa nini?** : Hulinda data yako dhidi ya ufikiaji wa kimwili usioidhinishwa au programu hasidi ya kunasa skrini.


#### 3.2.2. Uunganisho kupitia Tor





- **Kazi**: Zuia trafiki ya mtandao kupitia **Tor**, mtandao usiojulikana ambao husimba miunganisho yako kwa njia fiche.
- **Kwa nini?**: Ficha IP yako Address na ulinde faragha yako, vyema ikiwa huamini mtandao wako (kwa mfano, Wi-Fi ya umma).
- **Hasara**: Inaweza kupunguza kasi ya programu kutokana na usimbaji fiche.
- **Pendekezo**: Washa Tor ikiwa usiri ni kipaumbele, lakini jaribu kasi ya muunganisho.


#### 3.2.3. Kuunganisha kwa node ya kibinafsi





- **Kazi**: Huunganisha programu kwa **node yako kamili ya Bitcoin** kupitia server ya **Electrum**.
- **Kwa nini?**: Hutoa udhibiti kamili wa data ya Blockchain, ikiondoa utegemezi kwenye server za Blockstream.
- **Sharti**: node ya Bitcoin iliyosanidiwa.
- **Pendekezo**: Watumiaji mahiri wanaotafuta uhuru wa juu zaidi.


#### 3.2.4. Uthibitishaji wa SPV





- **Kazi**: Hutumia **Simple pay verification (SPV)** ili kuthibitisha moja kwa moja data fulani ya Blockchain bila kupakua msururu mzima.
- **Kwa nini?**: Hupunguza utegemezi wa node chaguomsingi ya Blockstream, huku ikisalia kuwa nyepesi kwa vifaa vya mkononi.
- **Hasara**: Ni salama kidogo kuliko Full node, kwani inategemea node za wahusika wengine kwa taarifa fulani.
- **Pendekezo**: Washa SPV ikiwa huwezi kutumia node ya kibinafsi, lakini pendelea Full node kwa usalama bora zaidi.





## 4. Kuunda kwingineko ya Bitcoin onchain



### 4.1. Anza uumbaji





- **Tahadhari**: Sanidi kwingineko yako katika mazingira ya faragha, bila kamera au waangalizi.
- Kutoka kwa skrini ya nyumbani, bonyeza "Anza":



![image](assets/fr/04.webp)





- Ikiwa ungependa kudhibiti **Cold Wallet** (Wallet ya nje ya mtandao): bofya **"Unganisha Jade "** ili kutumia Hardware Wallet Blockstream Jade au wallet zingine zinazooana za Cold.



![image](assets/fr/05.webp)





- Skrini inayofuata inaonekana:



![image](assets/fr/06.webp)





- (1) **"Weka Simu ya Mkononi Wallet"** : Unda Hot Wallet mpya (Hot Wallet).
- (2) **"Rejesha kutoka kwa Hifadhi Nakala "**: Ingiza kwingineko iliyopo kwa kutumia maneno ya Mnemonic (maneno 12 au 24). Tahadhari: Usiingize maneno ya Cold Wallet, kwani yatafichuliwa kwenye kifaa kilichounganishwa, na hivyo kubatilisha usalama wake.
- (3) **"Tazama Pekee "**: Leta jalada lililopo la kusoma tu, ili kuona salio (k.m. ya Cold Wallet yako) bila kufichua kifungu cha maneno cha Mnemonic. Tazama mafunzo ya Tazama Pekee katika kiambatisho.



**Katika somo hili**: Bofya **"Weka Simu ya Mkononi Wallet"** ili kuunda Hot Wallet.


Wallet yako inaundwa kiotomatiki na ukurasa wa nyumbani wa Wallet, unaoitwa hapa "My Wallet 5", unaonyeshwa:



![image](assets/fr/07.webp)



**Muhimu**: Programu ya Blockstream imerahisisha uundaji wa Wallet kwa kutoonyesha kiotomati maneno ya maneno 12 ya seed. *Ingawa kwingineko sasa imeundwa kwa mbofyo mmoja, unaweza kupoteza ufikiaji wa pesa zako ikiwa hutahifadhi maneno yako ya seed*.



### 4.2. Hifadhi sentensi ya seed





- Kwenye skrini ya kwanza ya Wallet, bofya kichupo cha "Usalama", kisha kwenye kidokezo cha "Hifadhi nakala" au menyu ya "Neno la Urejeshi":



![image](assets/fr/08.webp)



Kifungu cha maneno cha seed chenye maneno 12 kitaonyeshwa ili uhifadhi.





- Andika kifungu chako cha urejeshi kwa uangalifu mkubwa. Iandike kwenye karatasi au chuma na uiweke mahali salama (mahali salama, nje ya mtandao). Maneno haya ndiyo njia yako ya pekee ya kufikia bitcoins zako endapo kifaa chako kitapoteza au kufutwa kwa programu.
- Pia ni muhimu kutambua kwamba mtu yeyote aliye na kifungu hiki anaweza kuiba bitcoins zako zote. Usiwahi kuihifadhi kidijitali:
 - Hakuna picha ya skrini
 - Hakuna wingu, barua pepe au chelezo za ujumbe
 - Hakuna kunakili/kubandika (hatari ya kuhifadhi kwenye ubao wa kunakili)



**! Jambo hili ni muhimu **. Kwa habari zaidi juu ya nakala rudufu:



https://planb.network/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

https://planb.network/courses/46b0ced2-9028-4a61-8fbc-3b005ee8d70f

### 4.3. Thibitisha sentensi ya seed



Kabla ya kutuma pesa kwa Address inayohusishwa na sentensi hii ya seed, lazima ujaribu nakala ya maneno yako 12.



Ili kufanya hivyo, tutaandika kumbukumbu, futa Wallet, uirejeshe kwa salama, na uangalie kuwa kumbukumbu haijabadilishwa.





- Kwenye skrini ya kwanza ya Wallet, bofya kichupo cha "Mipangilio", kisha kwenye "Maelezo ya Wallet", na unakili zPub ([iliyoongezwa umma ufunguo](https://planb.network/courses/46b0ced2-9028-4a61-8fbc-3b005ee8d70f/8dcffce1-31bd-5e0b-965b-735f5f9e4602):



![image](assets/fr/09.webp)



Kumbuka: zpub Address inaweza kuingizwa kwenye programu yako ya Blockstream kwa kipengele cha "watch-only" (ona Kiambatisho).





- Futa programu, kisha urejeshe Wallet kupitia **"Rejesha kutoka kwa Hifadhi nakala "** kwa kuingiza kifungu cha Mnemonic, na uangalie kuwa zpub haijabadilishwa. Ikiwa ndio, basi nakala yako ni sahihi, na unaweza kutuma pesa kwa Wallet.





- Ili kupata maelezo zaidi kuhusu jinsi ya kufanya jaribio la urejeshi, hapa kuna mafunzo maalum:



https://planb.network/tutorials/wallet/backup/recovery-test-5a75db51-a6a1-4338-a02a-164a8d91b895

### 4.5. Kulinda ufikiaji wa programu



Funga ufikiaji wa programu kwa nambari thabiti ya PIN:




- Kutoka kwa skrini ya kwanza ya Wallet, nenda kwa **"Usalama"** kisha ubofye **"PIN "**
- Ingiza na uthibitishe **msimbo wa PIN wenye tarakimu 6 bila mpangilio**.



**Chaguo la kibayometriki**: Inapatikana kwa urahisi zaidi, lakini ni salama chini kuliko msimbo thabiti wa PIN (hatari ya ufikiaji usioidhinishwa, k.m. alama ya vidole au kuchanganua uso wakati wa kulala).



**Kumbuka**: PIN hulinda kifaa, lakini maneno ya seed pekee ndiyo yanaweza kutumika kupata pesa.





## 5. Kutumia onchain Wallet



### 5.1. Pokea bitcoins





- Kutoka kwenye skrini ya kwanza ya kwingineko, bofya kwenye '"**Shughulika**" kisha **"Pokea "**.



![image](assets/fr/10.webp)





- Programu inaonyesha **mapokezi tupu Address** (umbizo la SegWit v0, linaloanza na `bc1q...`). Kutumia Address mpya kila wakati unapopokea Bitcoin huboresha usiri wako.





- **Chaguzi** :
    - (1) "Bitcoin": bofya ili kuchagua usafirishaji wa onchain au Liquid, na uchague kipengee.
    - (2) Bofya kwenye mishale ili kuchagua Address nyingine mpya iliyounganishwa na sentensi hii ya seed.
    - (3) Unaweza pia kuchagua Address kutoka miongoni mwa zile ambazo tayari zimetumika/zinazoonyeshwa, kwa kubofya vitone vitatu kwenye sehemu ya juu kulia na kisha kwenye "Orodha ya Address"
    - (4) Ili kuomba kiasi mahususi, bofya vitone vitatu kwenye sehemu ya juu kulia, chagua "Omba kiasi", na uweke kiasi unachotaka. QR itasasishwa, na nafasi ya Address itachukuliwa na URI ya malipo ya Bitcoin.




![image](assets/fr/11.webp)





- Shiriki Address/URI kwa kubofya "**Shiriki**", kunakili maandishi au kuchanganua msimbo wa QR.
- **Uthibitishaji**: Angalia Address iliyoshirikiwa na mpokeaji kadiri uwezavyo ili kuepuka hitilafu au mashambulizi (k.m. programu hasidi kurekebisha ubao wa kunakili).



### 5.2. kutuma bitcoins





- Kutoka kwenye skrini ya kwanza ya kwingineko, bofya "**Transact**" kisha **"Tuma "** :



![image](assets/fr/12.webp)





- **Ingiza maelezo** :
    - (1) Weka **Address ya mpokeaji** kwa kuibandika au kuchanganua msimbo wa QR.
    - (2) Angalia mali na akaunti ambayo fedha zinatumwa.
    - (3) Onyesha **kiasi** kitakachotumwa. Unaweza kuchagua kitengo: BTC, satoshis, USD, ...


Kiasi cha chini (kikomo cha dush) mnamo 03/08/2025 ni 546 Sats.




    - (4) Chagua **ada za muamala** :
        - Chagua kutoka kwa chaguo zilizopendekezwa (k.m. haraka, wastani, polepole) kulingana na udharura, na takriban muda wa uhamisho utaonyeshwa.
        - Kwa gharama zilizobinafsishwa, rekebisha wewe mwenyewe nambari ya Satoshi kwa kila vbyte (angalia [Mempool.space](https://Mempool.space/) kwa viwango vya soko).




![image](assets/fr/13.webp)





- **Angalia** :
    - Angalia Address, kiasi na malipo kwenye skrini ya muhtasari.
    - Hitilafu ya Address inaweza kusababisha hasara isiyoweza kutenduliwa ya fedha. Jihadhari na programu hasidi ambayo hurekebisha ubao wa kunakili.



![image](assets/fr/14.webp)





- **Uthibitishaji**: Telezesha kitufe cha "Tuma" ili kusaini na kusambaza muamala.
-**Ufuatiliaji**: Katika kichupo cha "Transact" cha Wallet, muamala unaonekana kama "unaosubiri" hadi uthibitisho (uthibitisho 1 hadi 6):



![image](assets/fr/15.webp)





- Maadamu muamala haujathibitishwa, kitendakazi cha "Replace by fee" (tazama Kiambatisho) hukuruhusu kuharakisha ushughulikiaji wake kwa kuongeza ada za muamala:



![image](assets/fr/16.webp)




## Viambatisho



### A1. Mafunzo mengine ya Blockstream



Kwa kutumia Liquid Network



https://planb.network/tutorials/wallet/mobile/blockstream-app-liquid-b3e4fb82-902e-4782-ad2b-a61ab05a543a

Inaleta na kufuatilia Wallet katika hali ya "Tazama Pekee".



https://planb.network/tutorials/wallet/mobile/blockstream-app-watch-only-66c3bc5a-5fa1-40ef-9998-6d6f7f2810fb

Toleo la eneo-kazi



https://planb.network/tutorials/wallet/desktop/blockstream-app-desktop-c1503adf-1404-4328-b814-aa97fcf0d5da


### A2. Ufafanuzi wa Replace-by-fee (RBF)



**Ufafanuzi**: Replace-by-fee (RBF) ni kipengele cha mtandao wa Bitcoin kinachoruhusu mtumaji kuharakisha uthibitishaji wa shughuli ya **onchain** kwa kukubali kulipa ada ya juu zaidi.



**Vikomo** :




- RBF haipatikani kwa miamala ya Liquid au lightning.
- Muamala wa awali lazima uweke alama kuwa unaendana na RBF unapoundwa, ambayo Blockstream App hufanya kiotomatiki.



**Maelezo zaidi:**




- [Faharasa](https://planb.network/fr/resources/glossary/rbf-replacebyfee)




### A3. Mbinu bora



Ili kutumia **Blockstream App** kwa usalama na kwa ustadi, fuata mapendekezo haya. Watakusaidia kulinda pesa zako, kuboresha miamala yako, na kuhifadhi usiri wako kwenye mitandao ya **Bitcoin (onchain)**, **Liquid**, na **lightning**.





- **Linda maneno yako ya urejeshi** :
 - Mafunzo: Kuhifadhi maneno yako ya Mnemonic



https://planb.network/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

https://planb.network/courses/46b0ced2-9028-4a61-8fbc-3b005ee8d70f




- **Tumia uthibitishaji salama** :
 - Washa **PIN thabiti** au **uthibitishaji wa kibayometriki** (alama ya vidole au utambuzi wa uso) ili kulinda ufikiaji wa programu.
 - Usishiriki kamwe PIN au data ya kibayometriki.





- **Linda faragha yako** :
 - generate mpya Address kwa kila onchain au Liquid mapokezi ili kupunguza ufuatiliaji kwenye Blockchain.
 - Washa vitendaji vya "Faragha Iliyoimarishwa", "Tor" na "SPV".
 - Kwa usiri wa hali ya juu, unganisha Wallet yako na node yako ya Bitcoin kupitia server ya Electrum badala ya kutumia node ya umma.





- **Chagua mtandao unaofaa zaidi mahitaji yako** :
 - **Onchain**: Inapendekezwa kwa uangalizi wa muda mrefu au miamala ya thamani kubwa (ada hazizingatiwi kuhusiana na kiasi).
 - **Liquid**: Tumia kwa uhamisho wa haraka, wa gharama nafuu na usiri ulioimarishwa.
 - **lightning**: Chagua uhamisho wa papo hapo, wa gharama nafuu kwa kiasi kidogo.





- **Angalia Address za usafirishaji kila wakati** :
 - Kabla ya kutuma pesa, angalia Address kwa uangalifu. Fedha zilizotumwa kwa Address zisizo sahihi zinapotea milele. Tumia nakala/bandika au kuchanganua msimbo wa QR, usiwahi kunakili/urekebishe Address kwa mkono.





- **Boresha gharama** :
 - Kwa miamala ya mtandaoni, chagua ada zinazofaa (polepole, kati, haraka) kulingana na dharura na msongamano wa mtandao.
 - Tumia Liquid, au lightning kwa kiasi kidogo.





- Sasisha programu




### A4. Rasilimali za ziada





- **Viungo rasmi:**
 - **[Tovuti rasmi](https://blockstream.com/)**
 - **[Msaada wa programu ya simu](https://help.blockstream.com/hc/en-us/categories/900000056183-Blockstream-Green/)** : uwekaji kumbukumbu na gumzo
 - **[GitHub](https://github.com/Blockstream/green_android)**





- **Zuia Wachunguzi :**
 - on chain : **[Mempool.space](https://Mempool.space/)**
 - Liquid : **[Maelezo ya Blockstream](https://blockstream.info/Liquid)**
 - lightning: **[1ML (Lightning Network)](https://1ml.com/)**





- **Mafunzo na mafunzo:** **[Plan ₿ Network](https://planb.network/)** :
 - Inalinda kifungu chako cha urejeshi



https://planb.network/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

https://planb.network/courses/46b0ced2-9028-4a61-8fbc-3b005ee8d70f




- **Liquid Network** :
 - **[Kamusi](https://planb.network/fr/resources/glossary/liquid-network)**



https://planb.network/courses/6d26bcff-51a3-405f-bcdd-9af8297ce727




- **Lightning Network** :
 - **[Kamusi](https://planb.network/fr/resources/glossary/lightning-network)**



https://planb.network/courses/34bd43ef-6683-4a5c-b239-7cb1e40a4aeb
