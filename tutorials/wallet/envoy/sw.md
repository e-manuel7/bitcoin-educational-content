---
name: Envoy
description: Kuanzisha na kutumia Pasipoti na programu ya Mjumbe
---
![cover](assets/cover.webp)


Mjumbe ni programu ya usimamizi ya Bitcoin Wallet iliyotengenezwa na Foundation. Imeundwa mahususi kwa matumizi na passport Hardware Wallet.


passport "*Kundi la 2*" lililowasilishwa katika mafunzo haya na programu ya Mjumbe ni mrithi wa "*Toleo la Mwanzilishi*". Inatofautishwa na muundo wake wa hali ya juu, skrini ya rangi yenye ubora wa juu, na kibodi halisi ya ergonomic. Inafanya kazi katika hali ya "*Air-Gap*", kuhakikisha kwamba funguo zako za faragha za Wallet zinasalia kutengwa kabisa, na mawasiliano yanawezekana kupitia kadi ya MicroSD au misimbo ya QR. Kifaa kina vifaa vya betri ya Nokia BL-5C inayoweza kutolewa, inayoweza kuchajiwa na uwezo wa 1200 mAh. Betri hii isiyo ya umiliki inaweza kubadilishwa kwa urahisi, kwani mfano wa BL-5C unapatikana sana katika maduka.


Kuhusu muunganisho, Pasipoti ina mlango wa MicroSD, mlango wa USB-C wa kuchaji, na kamera ya nyuma ya kuchanganua misimbo ya QR.


Kwa upande wa usalama, Passport ina kipengele salama, na msimbo wa chanzo wa kifaa ni wazi kabisa. Inatoa vipengele vyote vinavyotarajiwa kutoka kwenye Bitcoin Hardware Wallet bora. Kumbuka kwamba Passport bado haitumii hati ndogo, lakini kipengele hiki kimepangwa kutekelezwa katika robo ya pili ya mwaka 2025.


Kwa bei ya $199, Pasipoti iko katika nafasi ya juu ya Hardware Wallet, ikishindana na Coldcard Q, Jade Plus, Tezor Safe 5 na modeli za juu zaidi za Ledger.


![Image](assets/fr/01.webp)


Ili kudhibiti Wallet yako salama kwenye Pasipoti, una chaguo kadhaa. Hardware Wallet hii inaoana na programu nyingi za usimamizi wa Wallet kwenye soko, ikiwa ni pamoja na Sparrow Wallet, Specter Desktop, Nunchuk, Keeper, miongoni mwa zingine.


Katika mafunzo haya, ambayo yanalenga wanaoanza na watumiaji wa kati, tutagundua jinsi ya kutumia programu ya Mjumbe na Pasipoti yako. Ndiyo njia rahisi ya kupata manufaa zaidi kutoka kwa Hardware Wallet yako.


Iwapo wewe ni mtumiaji wa hali ya juu na unataka kuchunguza vipengele ngumu zaidi, ninapendekeza uangalie mafunzo haya mengine ambapo tunasanidi Passport na Sparrow Wallet:


https://planb.network/tutorials/wallet/hardware/passport-74e53858-3fa2-43f9-b866-573297546236

## Kuondoa pasipoti


Unapopokea Pasipoti yako, hakikisha kisanduku na Seal kwenye katoni ni sawa ili kuthibitisha kwamba kifurushi hakijafunguliwa. Uthibitishaji wa programu wa uhalisi na uadilifu wa kifaa pia utafanywa kitakapowekwa.


![Image](assets/fr/02.webp)


Yaliyomo kwenye kisanduku ni pamoja na:




- Pasipoti;
- Kipande cha kadibodi cha kuandika maneno yako ya Mnemonic;
- Kebo ya USB-C ya kuchaji;
- Kadi ya MicroSD;
- Adapta mbili za MicroSD hadi Umeme au USB-C;
- Vibandiko.


Kwenye kifaa utapata:




- Kibodi (1);
- bandari ya USB-C (2);
- Kitufe cha kufuta (3);
- Kitufe cha kurudi (4);
- Kitufe cha uthibitisho (5);
- Pedi ya mwelekeo (6);
- Kitufe cha kuwasha/kuzima (7);
- Kiashiria cha hali (8);
- Bandari ya MicroSD (9);
- Kitufe cha kubadilisha hali aA1 (10);
- Kamera ya nyuma.


![Image](assets/fr/03.webp)


## Pakua programu ya Mjumbe


Nenda kwenye duka lako la programu ili kupakua Envoy :




- Kwenye [Google Play Store](https://play.google.com/store/apps/details?id=com.foundationdevices.envoy);
- Kwenye [App Store](https://apps.apple.com/us/app/envoy-by-foundation/id1584811818);
- Kwenye [F-Cold](https://foundation.xyz/fdroid/).


![Image](assets/fr/50.webp)


Unaweza pia kupakua faili ya APK moja kwa moja [kutoka hazina ya GitHub ya Foundation](https://github.com/Foundation-Devices/envoy/releases).


![Image](assets/fr/51.webp)


Mara tu programu imefunguliwa, chagua "*Dhibiti Pasipoti*".


![Image](assets/fr/52.webp)


Chagua ikiwa ungependa kuwezesha muunganisho wa Tor ili kuimarisha usiri, kisha ubonyeze "*Endelea*".


![Image](assets/fr/53.webp)


Chagua "*Unganisha passport iliyopo*" ikiwa Pasipoti yako tayari imesanidiwa, au "*Weka Pasipoti mpya*" ikiwa unaanzisha Hardware Wallet yako kwa mara ya kwanza.


![Image](assets/fr/54.webp)


Kubali masharti ya matumizi.


![Image](assets/fr/55.webp)


Kisha utaulizwa kuthibitisha uhalisi wa passport. Bonyeza "*Next*".


![Image](assets/fr/56.webp)


## Kuanzisha Pasipoti


Bonyeza kitufe cha kuwasha/kuzima kwenye upande wa kitengo ili kukianzisha.


![Image](assets/fr/04.webp)


Bonyeza kitufe cha uthibitishaji ili kufikia menyu inayofuata.


![Image](assets/fr/05.webp)


Katika somo hili, tutatumia Mjumbe kudhibiti Wallet inayolindwa na Pasipoti. Chagua "*Programu ya Mjumbe*".


![Image](assets/fr/57.webp)


Bofya kwenye "*Endelea kwa Mjumbe*".


![Image](assets/fr/58.webp)


Hatua inayofuata ni kuangalia kifaa chako. Hii inathibitisha uhalisi wa Pasipoti yako na inahakikisha kwamba haijaingiliwa katika usafiri. Utaulizwa kuchanganua msimbo wa QR.


![Image](assets/fr/08.webp)


Changanua misimbo ya QR inayobadilika inayoonyeshwa kwenye programu kwa kutumia Pasipoti yako. Wakati skanning imekamilika, bofya "*Next*".


![Image](assets/fr/59.webp)


Kisha utumie simu yako kuchanganua msimbo wa QR unaoonyeshwa kwenye Pasipoti yako.


![Image](assets/fr/60.webp)


Ikiwa ujumbe "*Pasipoti yako ni salama*" inaonekana, hii inathibitisha kwamba Hardware Wallet yako ni ya kweli. Sasa unaweza kuitumia kupata Bitcoin Wallet.


![Image](assets/fr/61.webp)


Thibitisha matokeo ya jaribio kwenye Pasipoti yako.


![Image](assets/fr/14.webp)


## Kuweka msimbo wa PIN


Inayofuata inakuja hatua ya msimbo wa PIN. Msimbo wa PIN hufungua Pasipoti yako. Kwa hiyo hutoa ulinzi dhidi ya ufikiaji wa kimwili usioidhinishwa. Msimbo wa PIN hauhusiki katika kupata vitufe vya kriptografia vya Wallet yako. Kwa hivyo hata bila ufikiaji wa msimbo wa PIN, umiliki wa maneno yako ya 12- au 24 ya Mnemonic itakuwezesha kupata tena ufikiaji wa bitcoins zako.


![Image](assets/fr/15.webp)


Tunapendekeza kuchagua msimbo wa PIN ambao ni nasibu iwezekanavyo. Pia, hakikisha umehifadhi msimbo huu katika sehemu tofauti na ambapo Pasipoti yako imehifadhiwa (k.m. katika kidhibiti cha nenosiri).


Unaweza kuchagua msimbo wa PIN kati ya tarakimu 6 na 12. Ninakushauri kuifanya kwa muda mrefu iwezekanavyo.


Tumia vitufe kuweka nambari zako za PIN. Unapomaliza, bofya kitufe cha uthibitishaji.


![Image](assets/fr/16.webp)


Thibitisha PIN yako mara ya pili.


![Image](assets/fr/17.webp)


Msimbo wako wa PIN umesajiliwa.


![Image](assets/fr/18.webp)


## Sasisha firmware ya Pasipoti


Hardware Wallet yako inapendekeza kwamba usasishe programu dhibiti yake. Ninapendekeza usasishe mara moja ili kufaidika na maboresho na marekebisho yanayoletwa na matoleo mapya zaidi. Ili kuendelea, bofya kitufe cha uthibitishaji upande wa kulia.


![Image](assets/fr/19.webp)


Pasipoti yako iko tayari kupokea programu dhibiti mpya kupitia kadi ya MicroSD.


![Image](assets/fr/20.webp)


### Bila maombi ya Mjumbe


Ili kufanya hivyo, tumia kadi ya MicroSD iliyojumuishwa kwenye sanduku la Pasipoti yako (au nyingine), na uiingiza kwenye kompyuta yako. Pakua toleo jipya zaidi la programu dhibiti kutoka [tovuti ya uhifadhi wa nyaraka za Foundation](https://docs.foundation.xyz/firmware-updates/passport/) au [hazina zao za GitHub](https://github.com/Foundation-Devices/passport2/releases).


![Image](assets/fr/21.webp)


Kabla ya kuiweka kwenye kifaa chako, tunakushauri sana uangalie uhalisi na uadilifu wa firmware iliyopakuliwa. Ikiwa unahitaji msaada kwa hili, soma mafunzo haya:


https://planb.network/tutorials/computer-security/data/integrity-authenticity-21d0420a-be02-4663-94a3-8d487f23becc

### Na maombi ya Mjumbe


Chaguo jingine, rahisi zaidi ni kutumia programu ya Mjumbe moja kwa moja. Bonyeza "*Pakua Firmware*".


![Image](assets/fr/62.webp)


Tumia adapta uliyopewa na Pasipoti yako kuunganisha kadi ya MicroSD kwenye simu yako.


![Image](assets/fr/63.webp)


Teua kadi ya MicroSD kwenye kichunguzi chako cha faili ili kuhifadhi firmware.


![Image](assets/fr/64.webp)


Firmware sasa imehifadhiwa. Ondoa MicroSD kutoka kwa smartphone yako na uiingiza kwenye Pasipoti.


![Image](assets/fr/65.webp)


Kichunguzi cha faili ya Pasipoti kitafungua. Chagua faili `vN.N.N-passport.bin`.


![Image](assets/fr/22.webp)


Bonyeza "* Chagua *".


![Image](assets/fr/23.webp)


Kisha thibitisha usakinishaji wa firmware.


![Image](assets/fr/24.webp)


Tafadhali subiri sasisho likamilike.


![Image](assets/fr/25.webp)


Baada ya kusasisha kukamilika, weka PIN yako ili kufungua kifaa na kuendelea na usanidi.


![Image](assets/fr/26.webp)


## Unda Bitcoin Wallet mpya


Sasa ni wakati wa kuunda Bitcoin Wallet mpya. Bonyeza kitufe cha uthibitisho.


![Image](assets/fr/27.webp)


Ili kuunda Wallet mpya, bofya "*Unda seed* Mpya".


![Image](assets/fr/28.webp)


Unaweza kuchagua kati ya maneno 12- au 24 ya Mnemonic. Usalama unaotolewa na chaguo zote mbili ni sawa, kwa hivyo unaweza kuchagua ile ambayo ni rahisi kuhifadhi, yaani maneno 12.


![Image](assets/fr/29.webp)


Bonyeza "*Endelea*".


![Image](assets/fr/30.webp)


Pasipoti yako sasa itakuwa generate "*Nambari ya Hifadhi nakala*". Huu ni mfululizo wa nambari zinazoweza kutumika kusimbua nakala rudufu ya Wallet yako iliyohifadhiwa kwenye MicroSD. Mfumo huu wa chelezo, mahususi kwa vifaa vya Foundation, unajumuisha nakala ya ziada ya maneno yako ya Mnemonic, lakini hauoani na programu nyingine za Bitcoin.


Ukiamua kutumia "*Nambari ya Hifadhi nakala*", hakikisha umeiweka katika eneo tofauti na MicroSD yako iliyo na hifadhi rudufu iliyosimbwa kwa njia fiche ya Wallet yako. Unaweza, hata hivyo, kuchagua kutotumia mfumo huu ikiwa unahisi kuwa nakala nzuri ya maneno yako ya Mnemonic inatosha.


![Image](assets/fr/31.webp)


Ingiza "*Nambari ya Hifadhi nakala*" ili kuthibitisha kuwa umeihifadhi kwa usahihi.


![Image](assets/fr/32.webp)


Ikiwa MicroSD imeingizwa, chelezo iliyosimbwa kwa njia fiche ya Wallet yako imehifadhiwa hapo.


![Image](assets/fr/33.webp)


Pasipoti yako itaonyesha maneno yako ya Mnemonic yenye maneno 12. Mnemonic hii inakupa ufikiaji kamili, usio na kikomo kwa bitcoins zako zote. Mtu yeyote aliye na kifungu hiki cha maneno anaweza kuiba pesa zako, hata bila ufikiaji wa kibinafsi wa Pasipoti yako.


Kifungu cha maneno 12 kinarejesha ufikiaji wa bitcoins zako katika tukio la kupoteza, kuibiwa au kuvunjika kwa Passport yako. Kwa hiyo ni muhimu sana kuihifadhi kwa uangalifu na kuiweka mahali salama.


Unaweza kuiandika kwenye kadibodi iliyotolewa kwenye kisanduku, au kwa usalama ulioongezwa, ninapendekeza kuiweka kwenye msingi wa chuma cha pua ili kuilinda kutokana na moto, mafuriko au kuanguka.


Bofya kwenye kitufe cha uthibitishaji ili kuona maneno yako ya Mnemonic.


![Image](assets/fr/34.webp)


Kwa maelezo zaidi kuhusu njia sahihi ya kuhifadhi na kudhibiti maneno yako ya Mnemonic, ninapendekeza sana kufuata mafunzo haya mengine, hasa ikiwa wewe ni mwanzilishi:


https://planb.network/tutorials/wallet/backup/backup-mnemonic-22c0ddfa-fb9f-4e3a-96f9-46e2a7954270

**bila shaka, hupaswi kamwe kushiriki maneno haya kwenye Mtandao, kama ninavyofanya katika mafunzo haya. Sampuli hii ya Wallet itatumika kwenye Testnet pekee na itafutwa mwishoni mwa mafunzo.**_


Weka nakala halisi ya sentensi hii.


![Image](assets/fr/35.webp)


Pasipoti yako imesanidiwa. Bofya kitufe cha uthibitishaji ili kuendelea.


![Image](assets/fr/36.webp)


## Kuanzisha Wallet kwenye Mjumbe


Katika somo hili, nitakuonyesha jinsi ya kutumia Pasipoti na programu ya Mjumbe. Walakini, Hardware Wallet hii pia inaendana na Sparrow Wallet, Keeper, BlueWallet, Nunchuk, Specter, na wengine wengi...


![Image](assets/fr/66.webp)


Tumia programu ya Mjumbe kuchanganua msimbo wa QR unaoonyeshwa kwenye Pasipoti yako.


![Image](assets/fr/67.webp)


Vifunguo vyako vya umma sasa vinaletwa kwenye programu. Bofya kwenye "*Thibitisha kupokea Address*".


![Image](assets/fr/68.webp)


Tumia Pasipoti yako kuchanganua Address inayoonyeshwa kwenye Mjumbe.


![Image](assets/fr/69.webp)


Pasipoti yako itathibitisha ikiwa Wallet iliyoingizwa kwa Mjumbe ni halali. Thibitisha katika programu.


![Image](assets/fr/70.webp)


Sasa unaweza kufikia maelezo ya umma ya Wallet yako kuhusu Mjumbe, lakini ili kutumia bitcoins, utahitaji kutumia Pasipoti yako.


![Image](assets/fr/71.webp)


## Gundua menyu za Pasipoti


Pasipoti yako Interface ina menyu kuu tatu:




- "*Account*";
- "*More*";
- "*Settings*".


Ili kusogeza kati ya menyu hizi, tumia vishale vya kushoto na kulia kwenye pedi ya mwelekeo.


### *Akaunti*" menyu


Katika menyu ya "*Account*", utapata vipengele vikuu vya Bitcoin Wallet yako. Unaweza kusaini muamala kupitia kamera au kupitia bandari ya MicroSD.


![Image](assets/fr/37.webp)


Menyu ndogo ya "*Account Tools*" inatoa chaguo kama vile kuthibitisha Address, kusaini ujumbe, au kushauriana na anwani katika Wallet yako.


![Image](assets/fr/38.webp)


Katika menyu ndogo ya "*Dhibiti Akaunti*", unaweza kuunganisha Bitcoin Wallet yako kwenye programu ya usimamizi ya Wallet (ambayo tutashughulikia katika hatua zinazofuata za mafunzo haya), au tazama na ubadilishe jina la akaunti yako.


![Image](assets/fr/39.webp)


### Menyu" zaidi


Katika menyu ya "*More*", unaweza kuunda akaunti mpya katika Wallet yako, iliyounganishwa na maneno sawa ya Mnemonic.


![Image](assets/fr/40.webp)


Unaweza pia kuingiza BIP39 passphrase au kutumia seed ya muda.


![Image](assets/fr/41.webp)


### Menyu ya Mipangilio


Katika menyu ya "*Settings*", utapata Wallet yako yote na mipangilio ya kifaa.


![Image](assets/fr/42.webp)


Menyu ndogo ya "*Kifaa*" hukupa chaguo za kubinafsisha mwangaza wa skrini, kuweka ucheleweshaji kabla ya kujifunga kiotomatiki, kubadilisha msimbo wa PIN, au kubadilisha jina la kifaa.


![Image](assets/fr/43.webp)


Menyu ndogo ya "*Chelezo*" hukuruhusu kutuma nakala rudufu yako ya Wallet iliyosimbwa kwa njia fiche, kuangalia uhalali wa chelezo iliyopo, au utafute "*Nambari ya Hifadhi nakala*" yako tena.


![Image](assets/fr/44.webp)


Menyu ndogo ya "*Firmware*" ni ya kusasisha programu dhibiti ya Pasipoti yako. Tunapendekeza ufanye masasisho haya mara kwa mara ili kufaidika na marekebisho na vipengele vya hivi punde.


![Image](assets/fr/45.webp)


Menyu ndogo ya "*Bitcoin*" inakuwezesha kubadilisha kitengo kinachoonyeshwa (BTC au satoshis), kudhibiti uwezekano wa Multisig Wallet, au ubadili hadi modi ya "*Testnet*".


![Image](assets/fr/46.webp)


Katika "*Advanced*", unaweza kuona maneno ya kifungu chako cha Mnemonic, kutekeleza vitendo kwenye MicroSD iliyoingizwa, kuweka upya Pasipoti yako kwenye mipangilio ya kiwandani, au kukagua uhalali, kama ulivyofanya awali.


![Image](assets/fr/47.webp)


Unaweza kuwezesha "*Maneno ya Usalama*", kipengele kinachoongeza Layer ya usalama kwa kuonyesha maneno mawili maalum wakati wa kufungua kifaa baada ya kuingiza tarakimu nne za kwanza za msimbo wa PIN. Maneno haya, ili kuhifadhiwa wakati wa usanidi, hakikisha kuwa Pasipoti haijabadilishwa au kuchezewa. Katika tukio la tofauti yoyote katika tarehe ya baadaye, tunakushauri usitumie kifaa. Ninakushauri kuamsha chaguo hili ili kuzuia hatari nyingi za maelewano ya kimwili ya kifaa.


![Image](assets/fr/48.webp)


Hatimaye, menyu ndogo ya "*Viendelezi*" hukuruhusu kuwezesha utendakazi mahususi kwa matumizi fulani ya kifaa, kama vile itifaki ya Whirlpool CoinJoin.


![Image](assets/fr/49.webp)


## Pokea bitcoins


Kwa kuwa sasa Pasipoti yako imesanidiwa, uko tayari kupokea Sats yako ya kwanza kwenye Bitcoin Wallet yako mpya. Ili kufanya hivyo, kwenye Mjumbe, bofya kwenye akaunti yako ya "*Msingi 0*".


![Image](assets/fr/72.webp)


Bonyeza kitufe cha "* Pokea *".


![Image](assets/fr/73.webp)


Programu yako ya Mjumbe huonyesha Address ya kwanza inayopatikana kwenye Wallet yako. Kabla ya kuitumia, hebu tuangalie Address kwenye skrini ya Pasipoti ili kuhakikisha kuwa ni mali ya Bitcoin Wallet yetu. Katika menyu ya "*Akaunti*" ya Pasipoti yako, chagua "*Zana za Akaunti*".


![Image](assets/fr/74.webp)


Bofya kwenye "*Thibitisha Address*", kisha uchanganue msimbo wa QR unaoonyeshwa kwenye Mjumbe.


![Image](assets/fr/75.webp)


Hakikisha kwamba Address iliyoonyeshwa kwenye Pasipoti inalingana kabisa na ile iliyoonyeshwa kwenye Sparrow, na kwamba "*Imethibitishwa*" inaonekana.


![Image](assets/fr/76.webp)


Sasa unaweza kuitumia kupokea bitcoins. Shughuli inapotangazwa kwenye mtandao, itaonekana kwenye Mjumbe. Subiri hadi upate uthibitisho wa kutosha ili kuzingatia muamala kuwa suluhu.


![Image](assets/fr/77.webp)


## Tuma bitcoins


Kwa kuwa sasa una Sats chache kwenye Wallet yako, unaweza pia kutuma baadhi. Ili kufanya hivyo, bofya kitufe cha "*Tuma*".


![Image](assets/fr/78.webp)


Ingiza Address ya mpokeaji, ama kwa kuibandika moja kwa moja, au kwa kuchanganua msimbo wa QR kwa kamera ya simu mahiri yako.


![Image](assets/fr/79.webp)


Bainisha kiasi unachotaka kutuma, kisha ubofye "*Thibitisha*".


![Image](assets/fr/80.webp)


Chagua ada ya ununuzi kulingana na hali ya sasa ya soko, kisha angalia maelezo ya muamala. Ikiwa kila kitu kiko sawa, bonyeza "* Ingia na Pasipoti*".


![Image](assets/fr/81.webp)


Ongeza lebo kwenye shughuli yako ya ununuzi ili kuweka rekodi wazi ya madhumuni yake.


![Image](assets/fr/82.webp)


Kisha Mjumbe anaonyesha PSBT (*Partially Signed Bitcoin Transaction*). Programu imeunda muamala, lakini bado inakosa saini za kufungua bitcoins zinazotumiwa kwenye ingizo. Sahihi hizi zinaweza tu kufanywa na Pasipoti, ambayo inapangisha seed yako ikitoa ufikiaji kwa funguo za faragha zinazohitajika ili kutia sahihi muamala.


![Image](assets/fr/83.webp)


Kwenye Pasipoti yako, fikia menyu ya "*Akaunti*" na ubofye "*Ingia kwa Msimbo wa QR*".


![Image](assets/fr/84.webp)


Changanua PSBT (*Partially Signed Bitcoin Transaction*) inayoonyeshwa kwenye Mjumbe.


![Image](assets/fr/85.webp)


Thibitisha kuwa Address inayopokea na kiasi kilichotumwa ni sahihi, kisha bonyeza kitufe cha uthibitishaji.


![Image](assets/fr/86.webp)


Angalia Exchange Address. Katika mfano wangu, hakuna, kwani shughuli hiyo inajumuisha pato moja.


![Image](assets/fr/87.webp)


Hakikisha ada ni ile uliyochagua.


![Image](assets/fr/88.webp)


Ikiwa maelezo yote ni sahihi, bofya kwenye kitufe cha uthibitisho ili kutia sahihi muamala.


![Image](assets/fr/89.webp)


Pasipoti yako hukuonyesha muamala wako uliotiwa saini kwa njia ya msimbo wa QR.


![Image](assets/fr/90.webp)


Kwenye programu ya Mjumbe, bofya aikoni ya msimbo wa QR, kisha uchanganue PSBT inayoonyeshwa kwenye skrini yako ya Pasipoti.


![Image](assets/fr/91.webp)


Angalia maelezo ya muamala wako kwa mara ya mwisho. Ikiwa kila kitu kiko sawa, bonyeza "*Tuma Muamala*" ili kuitangaza kwenye mtandao wa Bitcoin.


![Image](assets/fr/92.webp)


Muamala wako sasa unangoja uthibitisho. Unaweza kufuata hali yake moja kwa moja kutoka kwa akaunti yako.


![Image](assets/fr/93.webp)


Hongera, sasa unajua jinsi ya kusanidi na kutumia Pasipoti na programu ya Mjumbe. Ikiwa umepata mafunzo haya kuwa ya manufaa, ningeshukuru ikiwa utaacha kidole gumba cha Green hapa chini. Jisikie huru kushiriki nakala hii kwenye mitandao yako ya kijamii. Asante kwa kushiriki!


Kwa habari zaidi, angalia mafunzo yetu juu ya programu ya Liana:


https://planb.network/tutorials/wallet/desktop/liana-306ef457-700c-4fdd-b07a-8fb7a8a29f04
